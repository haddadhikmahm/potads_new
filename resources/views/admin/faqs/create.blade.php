...
@section('header_breadcrumb', 'DASHBOARD > FAQ > TAMBAH FAQ')
...
<div class="max-w-5xl">
    ...
    <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-8 md:p-12">
        <form action="{{ route('admin.faqs.store') }}" method="POST" class="space-y-10">
            @csrf
            ...
            <div class="space-y-8">
                ...
                <div class="space-y-4">
                    ...
                    <textarea name="answer" rows="8" required
                        ...
                        placeholder="Workshop ini bertujuan untuk memberikan ruang bagi anak-anak dengan Down Syndrome...">{{ old('answer') }}</textarea>
                    @error('answer') <p class="text-red-500 text-xs mt-1 ml-1">{{ $message }}</p> @enderror
                </div>

                <!-- Order Priority (Added because it was in index) -->
                    ...
                    <div class="space-y-4 flex flex-col justify-end">
                        <div class="flex items-center gap-4 px-8 py-5 bg-slate-50 rounded-2xl">
                            <label class="flex items-center cursor-pointer gap-3">
                                <input type="checkbox" name="is_active" value="1" checked class="w-5 h-5 rounded border-slate-300 text-potads-blue focus:ring-potads-blue">
