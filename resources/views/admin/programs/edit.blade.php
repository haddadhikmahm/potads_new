...
@section('header_breadcrumb', 'DASHBOARD > PROGRAM > EDIT')
        ...
        <div class="lg:col-span-2 space-y-6">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3" placeholder="Tuliskan deskripsi singkat untuk di halaman depan..." 
                                  class="w-full px-8 py-6 bg-slate-50 border border-slate-100 rounded-[2rem] focus:outline-none focus:ring-2 focus:ring-potads-blue/5 text-sm resize-none @error('description') border-red-500 @enderror">{{ old('description', $program->description) }}</textarea>
            ...
            <div class="bg-white rounded-[2.5rem] p-10 shadow-sm border border-slate-50">
                ...
                <textarea name="content" id="content" rows="12" placeholder="Tuliskan isi lengkap program..." 
                          class="w-full px-8 py-6 bg-slate-50 border border-slate-100 rounded-[2rem] focus:outline-none focus:ring-2 focus:ring-potads-blue/5 text-sm @error('content') border-red-500 @enderror">{{ old('content', $program->content) }}</textarea>
            ...
            <div class="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-50 sticky top-8">
                ...
                <div class="relative group">
                    ...
                    <label for="image-upload" class="flex flex-col items-center justify-center py-12 px-6 border-2 border-dashed border-slate-200 rounded-[2rem] cursor-pointer group-hover:border-potads-blue group-hover:bg-blue-50/30 transition-all duration-300">
                        @if($program->image)
                            <img id="image-preview" src="{{ asset('storage/' . $program->image) }}" class="w-full h-auto rounded-xl object-cover shadow-sm">
                            <div id="preview-container" class="hidden">
                                ...
                                <p class="text-xs font-bold text-slate-500 mb-1">Klik untuk ganti gambar</p>
                            </div>
                        @else
