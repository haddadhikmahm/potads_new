...
@section('title', 'Buat Artikel - PIK POTADS')
    ...
    <main class="max-w-7xl mx-auto px-6 md:px-12 py-8">
        ...
        <div class="text-center mb-12">
            ...
            <p class="text-gray-500 max-w-lg mx-auto leading-relaxed">
                Lengkapi data berikut untuk membagikan cerita atau informasi mengenai Down Syndrome. Kontribusi Anda sangat berarti bagi komunitas.
            </p>
        </div>
        ...
        <!-- Form Grid -->
        <form action="{{ route('articles.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            @csrf
            <!-- Left Side: Inputs -->
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="bg-blue-50/30 rounded-[2rem] p-8 border border-blue-50">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="title" value="{{ old('title') }}" required placeholder="Masukkan judul menarik..." class="w-full bg-white border border-gray-200 rounded-xl py-3 px-6 text-sm focus:outline-none focus:ring-2 focus:ring-potads-blue/20 transition">
                        </div>
                        <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="category" value="{{ old('category') }}" placeholder="Edukasi, Cerita, dll..." class="w-full bg-white border border-gray-200 rounded-xl py-3 px-6 text-sm focus:outline-none focus:ring-2 focus:ring-potads-blue/20 transition">
                            </div>
                            <div>
                                ...
                                <p class="text-[10px] text-gray-400 mt-2">Artikel Anda akan ditinjau oleh tim admin sebelum diterbitkan secara publik.</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-blue-50/30 rounded-[2rem] p-8 border border-blue-50">
                    ...
                    <div>
                        <textarea name="content" required placeholder="Tuliskan isi artikel yang ingin Anda bagikan..." class="w-full bg-white border border-gray-200 rounded-[2rem] py-6 px-8 text-sm focus:outline-none focus:ring-2 focus:ring-potads-blue/20 transition min-h-[300px]">{{ old('content') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="bg-blue-50/30 rounded-[2rem] p-8 border border-blue-50">
                    ...
                    <div class="border-2 border-dashed border-gray-300 rounded-[1.5rem] bg-white p-8 text-center cursor-pointer hover:border-potads-blue transition group relative" onclick="document.getElementById('image-upload').click()">
                        ...
                        <div id="image-preview" class="hidden absolute inset-0 bg-white rounded-[1.5rem] overflow-hidden">
                ...
                <div class="flex flex-col gap-4">
                    ...
                    <a href="{{ route('articles.index') }}" class="w-full bg-blue-50 text-potads-blue font-bold py-4 rounded-full text-center hover:bg-blue-100 transition">
                        Batal
                    </a>
                </div>
